...
class Logger(object):
    ...
    def __init__(self, filename, level='info', when='D', backCount=3, fmt='%(asctime)s - %(pathname)s - [functionName:%(funcName)s line:%(lineno)d] [process:%(process)d] [thread:%(thread)d threadName:%(thread)s] - %(levelname)s: %(message)s'):
